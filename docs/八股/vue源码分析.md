...
6.genAssignmentCode首先对 `v-model` 对应的 `value` 做了解析，它处理了非常多的情况，对我们的例子，`value` 就是 `messgae`，所以返回的 `res.key` 为 `null`，然后我们就得到 `${value}=${assignment}`，也就是 `message=$event.target.value`
...
`compiler/directives/model.js` 中定义的 `model` 函数    然后执行genComponentModel(el, value, modifiers);
...
slot实际上就相当于占位，然后对slot对应的内容进行替换 
...
6.resolveScopedSlots中fns是一个数组，每一个数组元素都有一个 `key` 和一个 `fn`，`key` 对应的是插槽的名称，`fn` 对应一个函数。整个逻辑就是遍历这个 `fns` 数组，生成一个对象，对象的 `key` 就是插槽名称，`value` 就是函数
...
4.==执行createComponent，这个时候 `isReactivated` 为 true==，并且在执行 `init` 钩子函数的时候不会再执行组件的 `mount` 过程了
