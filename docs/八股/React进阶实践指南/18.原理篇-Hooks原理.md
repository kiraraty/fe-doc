...
之前章节讲过，类组件的状态比如 state ，context ，props 本质上是存在类组件对应的 fiber 上，包括生命周期比如 componentDidMount ，也是以副作用 effect 形式存在的。那么 Hooks 既然赋予了函数组件如上功能，所以 hooks 本质是离不开**函数组件对应的 fiber** 的。 hooks 可以作为函数组件本身和函数组件对应的 fiber 之间的沟通桥梁。

![image-20220924111036194](https://femarkdownpicture.oss-cn-qingdao.aliyuncs.com/Imgs/image-20220924111036194.png)
...
workInProgress 正在调和更新函数组件**对应的 fiber 树**。

-   对于**类组件 fiber** ，**用 memoizedState 保存 state 信息**，**对于函数组件 fiber ，用 memoizedState 保存 hooks 信息**。
...
![image-20220924111844485](https://femarkdownpicture.oss-cn-qingdao.aliyuncs.com/Imgs/image-20220924111844485.png)
...
![image-20220924111928671](https://femarkdownpicture.oss-cn-qingdao.aliyuncs.com/Imgs/image-20220924111928671.png)

第二次复用时候已经发现 hooks 类型不同 `useState !== useRef` ，那么已经直接报错了。所以开发的时候一定注意 hooks 顺序一致性。

报错内容：

![image-20220924111940656](https://femarkdownpicture.oss-cn-qingdao.aliyuncs.com/Imgs/image-20220924111940656.png)
...
![image-20220924112033475](https://femarkdownpicture.oss-cn-qingdao.aliyuncs.com/Imgs/image-20220924112033475.png)
...
![image-20220924112048018](https://femarkdownpicture.oss-cn-qingdao.aliyuncs.com/Imgs/image-20220924112048018.png)
