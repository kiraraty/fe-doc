...
FMP（First Meaningful Paint）是 Web 性能指标之一，用于衡量页面首次渲染出有意义内容的时间点。类似的渲染相关指标包括：

### 1. **FMP（First Meaningful Paint）**
   - **概念**：指浏览器首次渲染出有意义内容（通常是主内容）的时间点。
   - **测量方式**：浏览器性能 API（如 `PerformanceObserver`）或 Lighthouse 评测。

### 2. **FP（First Paint）**
   - **概念**：页面首次渲染的时间，即浏览器绘制出任何像素的时间（可能只是背景色）。
   - **测量方式**：`performance.getEntriesByType('paint')`

### 3. **FCP（First Contentful Paint）**
   - **概念**：页面首次渲染出任何内容（如文本、图片、SVG）的时间。
   - **测量方式**：与 FP 类似，通过 `PerformanceObserver` 监听 `paint` 事件。

### 4. **LCP（Largest Contentful Paint）**
   - **概念**：页面中最大的可见内容（如主图、标题）渲染完成的时间，通常用来衡量加载体验。
   - **测量方式**：使用 `PerformanceObserver` 监听 `largest-contentful-paint` 事件。

### 5. **TTI（Time to Interactive）**
   - **概念**：页面变得可交互的时间，即主要内容加载完毕、JavaScript 事件监听已绑定，且页面对用户输入响应合理的时间点。
   - **测量方式**：Lighthouse 评测、`PerformanceObserver`。

### 6. **TBT（Total Blocking Time）**
   - **概念**：FCP 和 TTI 之间所有超过 50ms 的任务阻塞时间总和。
   - **测量方式**：Lighthouse 评测。

### 7. **CLS（Cumulative Layout Shift）**
   - **概念**：页面在加载过程中发生的累计布局偏移量（防止元素跳动影响用户体验）。
   - **测量方式**：`PerformanceObserver` 监听 `layout-shift` 事件。

---

### 如何获取 FMP 和其他指标
可以使用 **Performance API** 或 **Lighthouse** 来获取这些指标：

#### 1. 通过 Performance API
```js
// 获取 FP 和 FCP
new PerformanceObserver((entryList) => {
    for (const entry of entryList.getEntries()) {
...
如果你的目标是优化页面渲染，可以：
- 使用 **WebP**、**AVIF** 图片格式减少图片加载时间。
- 通过 **lazy loading** 延迟加载非关键资源。
- 采用 **SSR/SSG** 技术减少客户端渲染时间。
- 减少 **JavaScript 阻塞**（拆分 Bundle、使用 Code-splitting）。
- 启用 **HTTP/2** 或 **CDN** 以加快资源加载。
